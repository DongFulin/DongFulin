...
class Member extends Model
{
    ...
    public function doLogin($data,$scene='Member.pwd_login'){
        ...
        if(!$getData){
            datamsg(400,$unregisterDesc,array('info'=>'unregister'));
        }

        $userInfo = $this->where($where)->field('id,appinfo_code,checked,shop_id,pid')->find();

        if($userInfo && $userInfo['checked'] == 1){

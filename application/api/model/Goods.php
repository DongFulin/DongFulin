...
class Goods extends Model
{
    ...
    public function getGoodsShowPrice($goodsId,$type='normal',$position="detail"){
        ...
        switch ($type){
            ...
            case 'seckill':
                if($goods['hasoption']){
                    $goodsOptionModel = new GoodsOption();
                    $minPriceOption = $goodsOptionModel->where(['goods_id'=>$goodsId,'is_seckill'=>1])->order('seckill_price ASC')->find();
                    $maxPriceOption = $goodsOptionModel->where(['goods_id'=>$goodsId,'is_seckill'=>1])->order('seckill_price DESC')->find();
                    if($minPriceOption['seckill_price'] != $maxPriceOption['seckill_price']  && $position == 'detail'){
                        $priceData['seckill_price'] = $minPriceOption['seckill_price'].'~'.$maxPriceOption['seckill_price'];
                    }else{
                        $priceData['seckill_price'] = $minPriceOption['seckill_price'];
                    }
                    if($minPriceOption['shop_price'] != $maxPriceOption['shop_price'] && $position == 'detail'){
                        $priceData['shop_price'] = $minPriceOption['shop_price'].'~'.$maxPriceOption['shop_price'];
                    }else{
                        $priceData['shop_price'] = $minPriceOption['shop_price'];
                    }
                }else{
                    $seckillModel = new Seckill();
                    $seckillPrice =  $seckillModel->where('goods_id', $goodsId)
            ...
            case 'assemble':
                if($goods['hasoption']){
                    $goodsOptionModel = new GoodsOption();
                    $minPriceOption = $goodsOptionModel->where(['goods_id'=>$goodsId,'is_assemble'=>1])->order('assemble_price ASC')->find();
                    $maxPriceOption = $goodsOptionModel->where(['goods_id'=>$goodsId,'is_assemble'=>1])->order('assemble_price DESC')->find();
                    if($minPriceOption['assemble_price'] != $maxPriceOption['assemble_price'] && $position == 'detail'){
                        $priceData['assemble_price'] = $minPriceOption['assemble_price'].'~'.$maxPriceOption['assemble_price'];
                    }else{
                        $priceData['assemble_price'] = $minPriceOption['assemble_price'];
                    }
                    if($minPriceOption['shop_price'] != $maxPriceOption['shop_price'] && $position == 'detail'){
                        $priceData['shop_price'] = $maxPriceOption['shop_price'].'~'.$minPriceOption['shop_price'];
                    }else{
                        $priceData['shop_price'] = $minPriceOption['shop_price'];
                    }
                }else{
    ...
    public function getShopGoods($shopId,$num){
        ...
        foreach ($goodsList as $k=>$v){
            $goodsList[$k]['thumb_url'] = url_format($v['thumb_url'],$webUrl,'?imageMogr2/thumbnail/300x300');
        }
        return $goodsList;
    }

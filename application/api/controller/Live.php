        ...
        $list = db('live')->where($where)
                          ...
                          ->each(function ($item, $key) {
                              ...
                              if ($item['status'] == 1) { // 直播中
                                  //$item['addressitem'] = 'https://'.$this->webconfig['playdomain'].'/live/'.$item['room'].'.flv';
                              ...
                              for ($i = 0; $i < 3; $i++) {
                                  if (empty($recommendGoods[$i])) {
                                      ...
                                      $recommendGoods[$i]['thumb_url'] = url_format($recommendGoods[$i]['thumb_url'], $this->webconfig['weburl'],'?imageMogr2/thumbnail/200x200');
                                  }
                              }
...
//                if(!$user_arr['shop_id']){
//                    datamsg(400,'对不起，请先提交开店申请');
//                }
        $list = db('live_gifts')->where(['is_delete' => 0])->field('id,name,point,pic,picgif,description')->select();
    ...
    public function liveinformation()
    {
        ...
        $data['bicount'] = db('live_givegift')->where(['shop_id' => $shop_id])->sum('redbi');
        //$data['ordercount'] = db('order')->where(['user_id'=>$result['user_id'],'shop_id'=>$shop_id,'state'=>1])->count();
        $data['system_notice'] = $this->webconfig['livenotice'];
        ...
        if ($data['live_status'] == 1) {
            $data['addressitem'] = 'https://' . $this->webconfig['playdomain'] . '/live/' . $live['room'] . '.flv';
            $data['transcribe']  = 0;
        } else {
            ...
            if (!empty($data['addressitem'])) {
                ...
                $data['transcribe']  = 0;
                $data['addressitem'] = $this->webconfig['default_live_video_url'];
            }
        }
    ...
    public function giftsranking()
    {
        ...
        $givefigts = db('live_givegift')->where(['shop_id' => $shop_id])->group('uid')->field('sum(redbi) as countredbi,uid')->order('countredbi desc')->limit(10)->select();

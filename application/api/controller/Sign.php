...
class Sign extends Common
{
    ...
    public function dosign()
    {
        ...
        if($type==1){
            ...
            model('member')->addLog($guize[$sign_info['sum']+1]['num'],'连续签到奖励+'.$guize[$sign_info['sum']+1]['num'],1,$userId);
            ...
            model('member')->addLog($config['reward_default_day'],'日常签到+'.$config['reward_default_day'],0,$userId);
            $this->addIntegral($userId,$config['reward_default_day'],12);
            datamsg(200,'签到成功，积分+'.$config['reward_default_day'],array('integral'=>$config['reward_default_day']+$integral['integral']));
        }
    }
}
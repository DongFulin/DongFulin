...
class Shops extends Common{
    ...
    public function getShopInfo(){
        ...
        $userId = Db::name('rxin')->where('token', $token)->value('user_id');
    ...
    public function getRecommendShops(){
        ...
        $shopList = $shopModel->getRecommendShops(4);

        $goodsModel = new GoodsModel();
        foreach ($shopList as $k=>$v){
            $shopList[$k]['logo'] = url_format($v['logo'],$webconfig['weburl'],'?imageMogr2/thumbnail/100x100');
            $goodsList = $goodsModel->getShopGoods($v['id'],3);
            foreach ($goodsList as $k2=>$v2){
                $goodsList[$k2]['thumb_url'] = url_format($v2['thumb_url'],$webconfig['weburl'],'?imageMogr2/thumbnail/200x200');
            }
            $shopList[$k]['goods_list'] = $goodsList;
        }
        $returnData['shop_list'] = $shopList;

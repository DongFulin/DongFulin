...
class Index extends Common{
    ...
    public function getIndexShowcase(){
        ...
        $data['seckill']['list'] = Seckill::getRecommendSeckill(2);
        foreach ($data['seckill']['list'] as $k=>$v){
            $data['seckill']['list'][$k]['thumb_url'] = url_format($v['thumb_url'],$webconfig['weburl'],'?imageMogr2/thumbnail/200x');
        ...
        $data['assemble']['list'] = Assemble::getRecommendAssemble(2);
        foreach ($data['assemble']['list'] as $k=>$v){
            $data['assemble']['list'][$k]['thumb_url'] = url_format($v['thumb_url'],$webconfig['weburl'],'?imageMogr2/thumbnail/200x');
        }
        ...
        $data['new_goods']['list'] = $goodsModel->getTagGoods('is_new',2);
        foreach ($data['new_goods']['list'] as $k=>$v){
            $data['new_goods']['list'][$k]['thumb_url'] = url_format($v['thumb_url'],$webconfig['weburl'],'?imageMogr2/thumbnail/200x');
        }
        ...
        $data['live']['list'] = $liveModel->getRecommendLiveRoom(2);
        foreach ($data['live']['list'] as $k=>$v){
            $data['live']['list'][$k]['cover'] = url_format($v['cover'],$webconfig['weburl'],'?imageMogr2/thumbnail/200x');
        }

...
class ApplyInfo extends Common{
    ...
    public function panduan(){
        ...
        $members = Db::name('member')->where('id',$userId)->field('phone,password')->find();
        if($members['phone']){
                                ...
                                if($areas){
                                    ...
                                    if($data['apply_type'] == 2){ // 企业店铺必填营业执照
                                        if(empty($data['zhizhao'])){
                                            $value = array('status'=>400,'mess'=>'请上传营业执照','data'=>array('status'=>400));
                                            return json($value);
                                        }
                                    }else{
                                        $data['zhizhao'] = '';
                                    ...
                                    try{
                                        ...
                                        // 回滚事务
                                        Db::rollback();
                                        $value = array('status'=>400,'mess'=>'提交资料失败','data'=>array('status'=>$e->getMessage()));
                                    }
                                }else{
...
//            $value = array('status'=>200,'mess'=>'获取入驻申请状态信息成功','data'=>$ruzhuinfos);
            datamsg(200,'获取入驻申请状态信息成功',$ruzhuinfos);
        }else{

...
class MemberOrder extends Common{
        ...
        if(in_array($filter,array(1,2,3,4,6,7))){
            ...
            if($orderes){
                foreach ($orderes as $k => $v){
                    if($v['state'] == 0 && $v['fh_status'] == 0 && $v['order_status'] == 0 && $v['is_show'] == 1){
                        ...
                        if($v['time_out'] <= time()){
                            ...
                            try{
                                ...
                                if($v['coupon_id']){
                                    Db::name('member_coupon')->where('user_id',$userId)->where('coupon_id',$v['coupon_id'])->where('is_sy',1)->where('shop_id',$v['shop_id'])->update(array('is_sy'=>0));
                                }
                    ...
                    foreach ($orderes[$k]['goodsinfo'] as $key => $val){
                        $orderes[$k]['goodsinfo'][$key]['thumb_url'] = url_format($val['thumb_url'],$webconfig['weburl'],'?imageMogr2/thumbnail/150x150');
            ...
            if($orderes){
                foreach ($orderes as $k => $v){
                    ...
                    $orderes[$k]['orgoods']['thumb_url'] = url_format($orderes[$k]['orgoods']['thumb_url'],$webconfig['weburl'],'?imageMogr2/thumbnail/150x150');
    ...
    public function getOrderInfo(){
        ...
        foreach ($orders['goodsinfo'] as $key => $val){
            $orders['goodsinfo'][$key]['thumb_url'] =url_format($val['thumb_url'],$this->webconfig['weburl'],'?imageMogr2/thumbnail/350x350');
    ...
    public function thorderinfo(){
        ...
        $orgoods['thumb_url'] = url_format($orgoods['thumb_url'],$webconfig['weburl'],'?imageMogr2/thumbnail/350x350');
        ...
        datamsg(200,'获取退换货申请信息成功',$thapplyinfo);
    }
    ...
    public function zhifuorder(){
                ...
                foreach ($orderGoodsList as $ok => $ov){
                    ...
                    if($activity){
                        if($activity['ac_type'] == 1){
                            ...
                            $hasBuyStr = $hasBuy > 0 ? '，您已经购买过'.$hasBuy.'件' : '';
                            if($ov['goods_num']+$hasBuy > $activity['xznum']){
                                datamsg(400,$ov['goods_name'].'-限购'.$activity['xznum'].'件'.$hasBuyStr);
                            }
                    ...
                    if($ov['goods_num'] <= 0 || $ov['goods_num'] > $stock){
                        datamsg(400,$ov['goods_name'].'库存不足');
                    }
                    unset($stock);
                    unset($hasBuy);
                }
        ...
        if(!$dingdan){
            ...
            try{
	            ...
	            datamsg(200,'获取订单信息成功',$orderinfos);
            } catch (\Exception $e) {
                // 回滚事务
                Db::rollback();
            ...
            if($orders){
                //if($orders['shouhou'] == 0){
                    ...
                    try{
						...
						$num3 = $this->getLiveIntegralRules(9);
						$this->addLiveIntegral($userId,$shopid,$room,$num3,9,$orders['id']);

						$num4 = $this->getLiveIntegralRules(10);
						$num5 = floor($total_price/100) * $num4;
						$this->addLiveIntegral($userId,$shopid,$room,$num5,10,$orders['id']);
...
//
                                ...
                                Db::name('shop_detail')->insert(array('de_type'=>1,'sr_type'=>1,'price'=>$total_price,'order_type'=>1,'order_id'=>$orders['id'],'shop_id'=>$orders['shop_id'],'wat_id'=>$watId,'time'=>time()));
						}

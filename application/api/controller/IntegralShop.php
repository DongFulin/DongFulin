...
class IntegralShop extends Common{
    ...
    public function getGoodsList() {
        ...
        foreach ($goods_list as $key => $v) {
            $goods_list[$key]['goods_img'] = url_format($v['goods_img'],$webconfig['weburl'],'?imageMogr2/thumbnail/350x350');
    ...
    public function getIntegralRecordList() {
        ...
        foreach ($record_list as $key => $v) {
            $record_list[$key]['task_name'] = $task_arr[$v['type']];
        }
    ...
    public function getIntegralByShare() {
        ...
        //1每日登录,2邀请注册,3完善信息（绑定手机）,4完善信息（上传头像）,5观看直播（30min）,6直播发言（次）,7直播分享（次）,8购物消费（%）,9订单评价（次）,10商品分享,11连续签到奖励,12普通签到奖励,13积分兑换

...
class Order extends Common
{
    ...
    public function cartbuy()
    {
        ...
        foreach ($cartIdRes as $v) {
            ...
            if ($activity) {
                if ($activity['ac_type'] == 1) {
                    ...
                    if ($carts['num'] + $hasBuy > $activity['xznum']) {
                        datamsg(400, $carts['goods_name'] . '-限购' . $activity['xznum'] . '件' . $hasBuyStr);
                    }
            ...
            if ($carts['num'] <= 0 || $carts['num'] > $stock) {
                datamsg(400, $carts['goods_name'] . '库存不足');
            }
            unset($stock);
            unset($hasBuy);
    ...
    public function cartsure()
    {
        ...
        foreach ($cartIdRes as $v) {
            ...
            $carts['thumb_url'] = url_format($carts['thumb_url'], $webconfig['weburl'], '?imageMogr2/thumbnail/350x350');
        ...
        if ((!$activity) || ($activity && $activity['ac_type'] == 3 && $fangshi == 1)) { // 非活动商品或拼团商品的单独购买
            ...
            if ($activity['ac_type'] == 1) {
                ...
                if ($num + $hasBuy > $activity['xznum']) {
                    datamsg(400, '该商品限购' . $activity['xznum'] . '件' . $hasBuyStr);
                }
    ...
    public function pursure()
    {
        ...
        $purchs['thumb_url'] = url_format($purchs['thumb_url'], $webconfig['weburl'], '?imageMogr2/thumbnail/300x300');
        ...
        if ((!$activity) || ($activity && $activity['ac_type'] == 3 && $fangshi == 1)) { // 非活动商品或拼团活动的单独购买
            ...
            if ($purchs['num'] > 0 && $purchs['num'] <= $stock) {
                ...
                datamsg(400, $purchs['goods_name'] . '库存不足');
            }
        }
    ...
    public function addorder()
    {
        ...
        //        if(!in_array(input('post.zf_type'),array(1,2,3,5))){
        ...
        $address = Db::name('address')
                     ->alias('a')
                     ->field('a.*,b.pro_name,c.city_name,d.area_name')
    ...
    public function zhifu()
    {
        ...
        if (!in_array(input('post.zf_type'), array(1,2,3,4,5))) {
            datamsg(400, '支付方式参数错误');
        }
        ...
        if ($scene == 'goods') {
            ...
            if (!$orderinfos) {
                datamsg(400, '找不到相关类型订单' . $orderNumber);
            }
            $orderes = Db::name('order')->where('zong_id', $orderinfos['id'])->field('id,ordernumber,state,fh_status,order_status,order_type,pin_type,pin_id,time_out,shop_id')->select();
            ...
            $orderIntegral = intval($orderinfos['integral']);
        ...
        switch ($zf_type) {
            ...
            case 2:
                ...
                if ($quxiao_time > 60) {
                    ...
                    if (input('post.wechat_miniprogram') == 1) {
                        ...
                        datamsg(200, '创建订单成功', array('order_number' => $orderinfos['order_number'], 'infos' => $wechatePayRes, 'order' => $order));
                    } else {
                        // APP微信支付
                        $wx = new WxPay();
                        ...
                        if ($order['prepay_id']) {
                            ...
                            datamsg(200, '创建订单成功', array('order_number' => $orderinfos['order_number'], 'infos' => $order1));
                        } else {
            ...
            case 5:
                ...
                if ($cards) {
                    ...
                    $res = db('order')->where('zong_id', $orderZongId)->update($zongData);
                    $data['order_number'] = $orderNumber;
                    $data['card_name'] = $card_name;
                    $data['card_number'] = $card_number;
                    $cardRes = db('order_card')->insert($data);
                    if ($cardRes) {

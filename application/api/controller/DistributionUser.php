...
//        );
    ...
    public function withdrawInfo() {
        ...
        $tip_str = "每月最多提现" . $withdraw_count . "次，每次最少提现" . $min_amount . "元";
        //是否设置支付密码
        $memberModel = new Member();
    ...
    public function withdrawSubmit() {
        ...
        if ($amount < $webconfig['commission_withdraw_min_money']) {
            datamsg(400, '每次最少提现'.$webconfig['commission_withdraw_min_money'].'元');
        }
        $distribbModel = new DistributionUserModel();
        ...
        if ($withdrawCount >= $webconfig['commission_withdraw_max_number']) {
            datamsg(400, '每月最多提现'.$webconfig['commission_withdraw_max_number'].'次');
        }

...
class Common extends Controller{
    ...
    public function _initialize(){
        ...
        $_configres = Db::name('config')->where('ca_id','in','1,2,5,8,11,15,16,17,18,21')->field('ename,value')->select();
        $configres = array();
     ...
     * @return: array
     */
    function checkToken($needUserToken = 1){
        if(request()->isPost()){
     ...
     * @description: 会员积分规则
     * @Author: lxb
     * @param : $type: 1每日登录,2邀请注册,3完善信息（绑定手机）,4完善信息（上传头像）,5观看直播（30min）,6直播发言（次）,7直播分享（次）,8购物消费（%）,9订单评价（次）,10商品分享,11连续签到奖励,12普通签到奖励,13积分兑换
     * @return: array
     */
     ...
     * @description: 增加会员积分
     * @Author: lxb
     * @param : $userId:用户id;$num:积分;$type:1每日登录,2邀请注册,3完善信息（绑定手机）,4完善信息（上传头像）,5观看直播（30min）,6直播发言（次）,7直播分享（次）,8购物消费（%）,9订单评价（次）,10商品分享,11连续签到奖励,12普通签到奖励,13积分兑换
     * @return: json
     */
     ...
     * @return: json
     */
    function decIntegral($userId,$num,$type,$order_id=0){
        $data['user_id'] = $userId;
        $data['integral'] = $num;
        $data['type'] = $type;
        $data['order_id'] = $order_id;
     ...
     * @return: json
     */
    function getLiveIntegralRules($type){
     ...
     * @return: json
     */
    function addLiveIntegral($userId,$shopid,$room,$num,$type,$order_id=0,$ping_id=0){

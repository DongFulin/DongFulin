...
class Address extends Common{
    ...
    public function index(){
        ...
        if(input('post.page') && preg_match("/^\\+?[1-9][0-9]*$/", input('post.page'))){
            ...
            $address = Db::name('address')
                         ->alias('a')
                         ->field('a.id,a.contacts,a.phone,a.address,a.is_default,a.datavalue,b.pro_name,c.city_name,d.area_name')
                         ->join('sp_province b','a.pro_id = b.id','INNER')
                         ->join('sp_city c','a.city_id = c.id','INNER')
                         ->join('sp_area d','a.area_id = d.id','INNER')
        ...
        if(true !== $yzresult){
            ...
            if($pros){
                ...
                if($citys){
                    ...
                    if(!$areas){
	                    datamsg(400, '请选择区县' ,array('status'=>400));
                    }
                }else{
	                datamsg(400, '请选择城市' ,array('status'=>400));
                }
            }else{
	            datamsg(400, '请选择省份' ,array('status'=>400));
            }
    ...
    public function getinfo(){
        ...
        if(input('post.dz_id')){
            ...
            $address = Db::name('address')->where('id',$dz_id)->where('user_id',$userId)->field('id,contacts,phone,pro_id,city_id,area_id,address,is_default')->find();
            if($address){
                $prores = Db::name('province')->field('id,pro_name,zm')->order('sort asc')->select();
                $cityres = Db::name('city')->where('pro_id',$address['pro_id'])->field('id,city_name,zm')->order('sort asc, id asc')->select();
                $areares = Db::name('area')->where('city_id',$address['city_id'])->field('id,area_name,zm')->order('sort asc, id asc')->select();

...
class Goods extends Common{
    ...
    public function goodsInfo(){
        ...
        $goodsModel = new GoodsModel();
        $webconfig = $this->webconfig;
        $goods['thumb_url'] = url_format($goods['thumb_url'],$webconfig['weburl'],'?imageMogr2/thumbnail/750x750');
        ...
        foreach ($gpres as $kp => $vp){
            $gpres[$kp]['img_url'] = url_format($vp['img_url'],$webconfig['weburl'],'?imageMogr2/thumbnail/750x750');
        }
        ...
        if (!empty($goods) && $goods['hasoption']) {
            ...
            foreach ($specs as &$spec) {
                ...
                foreach ($spec['list'] as &$specItem){
                    $specItem['thumb'] = !empty($specItem['thumb']) ? url_format($specItem['thumb'],$webconfig['weburl'],'?imageMogr2/thumbnail/100x100') : '';
                }
                unset($specItem);
            }
            unset($spec);
        ...
        $shopinfos = Db::name('shops')
                       ...
                       ->find();
        $shopinfos['logo'] = url_format($shopinfos['logo'],$webconfig['weburl'],'?imageMogr2/thumbnail/350x350');

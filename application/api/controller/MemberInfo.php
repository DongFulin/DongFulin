...
class MemberInfo extends Common{
    ...
    public function readprofile(){
        ...
        $members = Db::name('member')->where('id',$userId)->field('id,user_name,phone,password,headimgurl,integral,sex,birth,email,oauth,shop_id')->find();
        if(!$members){
        ...
        $members['rz_zt'] = 0;
        if($members['phone'] && $members['shop_id'] > 0){
    ...
    public function editprofile(){
        ...
        if(true !== $yzresult){
            ...
            try{
                ...
                // 提交事务
                Db::commit();
                $value = array('status'=>200, 'mess'=>'设置成功', 'data'=>array('status'=>200),'info'=>$datainfo);
            } catch (\Exception $e) {
                // 回滚事务
                Db::rollback();

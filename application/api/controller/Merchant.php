                        ...
                        if($v['state'] == 0 && $v['fh_status'] == 0 && $v['order_status'] == 0 && $v['is_show'] == 1){
                            ...
                            if($v['time_out'] <= time()){
                                ...
                                try{
                                    ...
                                    $goodinfos = Db::name('order_goods')
                                                   ->where('order_id',$v['id'])
                                                   ->field('id,goods_id,goods_attr_id,goods_num,hd_type,hd_id')
                                                   ->select();
                                    if($goodinfos){
                                        foreach ($goodinfos as $val3){
                                            if(in_array($val3['hd_type'],array(0,2,3))){
                                                $prokc = Db::name('product')
                                                           ->where('goods_id',$val3['goods_id'])
                                                           ->where('goods_attr',$val3['goods_attr_id'])
                                                           ->find();
                                                if($prokc){
                                                    Db::name('product')
                                                      ->where('goods_id',$val3['goods_id'])
                                                      ->where('goods_attr',$val3['goods_attr_id'])
                                                      ->setInc('goods_number', $val3['goods_num']);
                                                }
                                    ...
                                    try{
                                        ...
                                        if($order_assembleres){
                                            foreach ($order_assembleres as $vrc){
                                                $pinorders = Db::name('order')
                                                               ->where('id',$vrc['order_id'])
                                                               ->where('state',1)
                                                               ->where('fh_status',0)
                                                               ->where('order_status',0)
                                                               ->where('order_type',2)
                                                               ->where('is_show',1)
                                                               ->field('id,total_price,user_id')
                                                               ->find();
                                                if($pinorders){
                                                    ...
                                                    $orgoods = Db::name('order_goods')
                                                                 ->where('order_id',$pinorders['id'])
                                                                 ->field('goods_id,goods_attr_id,goods_num,hd_type,hd_id')
                                                                 ->find();
                                                    if($orgoods){
                                                        Db::name('product')
                                                          ->where('goods_id',$orgoods['goods_id'])
                                                          ->where('goods_attr',$orgoods['goods_attr_id'])
                                                          ->setInc('goods_number', $orgoods['goods_num']);
                                                    }
                                                }
                                            }
                                        }
                            ...
                            if($v['shouhou'] == 0 && $v['zdsh_time'] <= time()){
                                ...
                                try{
                                    ...
                                    $goodinfos = Db::name('order_goods')
                                                   ->where('order_id',$v['id'])
                                                   ->field('id,goods_id,goods_attr_id,goods_num,th_status,shop_id')
                                                   ->select();
                                    if($goodinfos){
                                        foreach ($goodinfos as $val2){
                                            if(in_array($val2['th_status'], array(0,8))){
                                                $gdinfos = Db::name('goods')
                                                             ->where('id',$val2['goods_id'])
                                                             ->field('id,sale_num,deal_num')
                                                             ->find();
                                                if($gdinfos){
                                                ...
                                                $spinfos = Db::name('shops')
                                                             ->where('id',$val2['shop_id'])
                                                             ->field('id,sale_num,deal_num')
                                                             ->find();
                                                if($spinfos){
        ...
        if(input('post.order_num')){
            ...
            if($orders['state'] == 0 && $orders['fh_status'] == 0 && $orders['order_status'] == 0 && $orders['is_show'] == 1 && $orders['time_out'] <= time()){
                ...
                try{
                    ...
                    if($goodinfos){
                        foreach ($goodinfos as $val3){
                            if(in_array($val3['hd_type'],array(0,3))){
                                $prokc = Db::name('product')
                                           ->where('goods_id',$val3['goods_id'])
                                           ->where('goods_attr',$val3['goods_attr_id'])
                                           ->find();
                            ...
                            }elseif($val3['hd_type'] == 2){
                                $hdactivitys = Db::name('integral_shop')->where('id',$val3['hd_id'])->find();
                                if($hdactivitys){
                                    Db::name('integral_shop')->where('id',$val3['hd_id'])->setInc('stock',$val3['goods_num']);
                                    Db::name('integral_shop')->where('id',$val3['hd_id'])->setDec('sold',$val3['goods_num']);
                                }
                            }
                        }
                    }
                    ...
                    $orders = Db::name('order')
                                ->alias('a')
                                ->field('a.id,a.ordernumber,a.contacts,a.telephone,a.province,a.city,a.area,a.address,a.goods_price,a.freight,a.youhui_price,a.coupon_id,a.total_price,a.beizhu,a.state,a.pay_time,a.fh_status,a.fh_time,a.order_status,a.shouhou,a.is_show,a.coll_time,a.can_time,a.ping,a.order_type,a.pin_type,a.pin_id,a.zong_id,a.shop_id,a.addtime,a.zdsh_time,a.time_out,b.order_number,c.shop_name')
                                ->join('sp_order_zong b','a.zong_id = b.id','LEFT')
                                ->join('sp_shops c','a.shop_id = c.id','LEFT')
                                ->where('a.ordernumber',$order_num)
                                ->where('a.user_id',$userId)

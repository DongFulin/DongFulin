...
class Seckill extends Common{
    ...
    public function gaibian(){
        ...
        $rushs = Db::name('seckill')->where('id',$id)->where('shop_id',$shop_id)->find();
        if($rushs){
            $data[$name] = $value;
            $data['id'] = $id;
            $count = Db::name('seckill')->update($data);
            if($count > 0){
                ys_admin_logs('修改秒杀活动推荐状态','seckill',$id);
                $result = 1;
            }else{
                $result = 0;
            }
        ...
        if(request()->isPost()){
            ...
            if(input('post.id')){
                ...
                if($goods['hasoption'] == 1){
                    ...
                    $goods['option'] = $goodsCommonModel->getSeckillGoodsOptionHtml($goods_id);
                }
    ...
    public function add(){
        if(request()->isPost()){
            ...
            if($goods['hasoption'] == 1){
                $scene = 'Seckill.hasoption';
            }else{
                $scene = 'Seckill.nooption';
            }
...
//                datamsg(0,'开始时间需大于当前时间');
                ...
                foreach ($optionId as $k=>$v){
                    if(in_array($v,$optionIdCheckedArr)){
                        ...
                        Db::name('goods_option')->where('id',$v)->update([
                            'is_seckill' => 1,
                            'seckill_price' => trim($optionPrice[$k]),
                            'seckill_stock' => trim($optionStock[$k])
                        ]);
                    }
                }
            ...
            $lastId = Db::name('seckill')->insert(array(
    ...
    public function info(){
        if(input('id')){
            ...
            $rushs = Db::name('seckill')->alias('a')->field('a.*,b.hasoption,b.total,b.goods_name,b.shop_price,b.min_price,b.max_price,b.thumb_url,c.shop_name')->join('sp_goods b','a.goods_id = b.id','LEFT')->join('sp_shops c','a.shop_id = c.id','LEFT')->where('a.id',input('id'))->where('a.shop_id',$shop_id)->find();
            if($rushs){
                if($rushs['hasoption']){
                    $goodsCommonModel = new GoodsCommonModel();
    ...
    public function delete(){
        ...
        $id = input('id');
        if(!empty($id)){
            $rushs = Db::name('seckill')->where('id',$id)->where('shop_id',$shop_id)->where('is_show',1)->find();
            if($rushs){
                ...
                try{
                    ...
                    $goodsOptionModel->unSetGoodsOptionSeckillData($rushs['goods_id']);
                    Db::name('seckill')->update(array('is_show'=>0,'id'=>$id));
                    ...
                    // 提交事务
                    Db::commit();
                    ys_admin_logs('平台删除秒杀活动','seckill',$id);

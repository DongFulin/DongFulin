...
class Livegifts extends Common{
    public function lst(){
        ...
        $where['a.is_delete']=0;
        $field = 'a.*,ag.cname';
        $list = Db::name('live_gifts')
            ->alias('a')
            ->field($field)
            ->join('live_giftscate ag','a.cid = ag.id','LEFT')
            ->where($where)
            ->order('a.id desc')
            ->paginate($limit)
            ->each(function ($item){
    ...
    public function add(){
        if(request()->isAjax()){
            ...
            if(true !== $result){
                ...
                $data['play']=5;
                $result = db('live_gifts')->insertGetId($data);
                if($result){
            ...
            $cateres = db('live_giftscate')->where(['is_delete'=>0])->field('id,cname')->select();
            $this->assign([
               'cateres'=>$cateres,
            ]);
            return $this->fetch();
    ...
    public function edit()
    {
        ...
        if (request()->isAjax()) {
            ...
            if(true !== $result){
                ...
                $result = db('live_gifts')->where(['id'=>$id])->update($data);
                if($result){
            ...
            $cateres = db('live_giftscate')->where(['is_delete' => 0])->field('id,cname')->select();
            $gifts = db('live_gifts')->where(['id'=>$id])->find();

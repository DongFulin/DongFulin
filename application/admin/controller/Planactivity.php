...
class Planactivity extends Basic{
    ...
    public function planhd(){
        ...
        //过期秒杀信息
        $end_rushres = Db::name('seckill')->where('checked',1)->where('hd_bs',1)->where('is_show',1)->where('end_time','elt',$nowtime)->field('id,goods_id')->select();
        if($end_rushres){
            foreach ($end_rushres as $vr){
                ...
                Db::startTrans();
                try{
                    Db::name('seckill')->update(array('hd_bs'=>2,'id'=>$vr['id']));
        ...
        //秒杀中信息
        $rushres = Db::name('seckill')->where('checked',1)->where('hd_bs',0)->where('is_show',1)->where('start_time','elt',$nowtime)->where('end_time','gt',$nowtime)->field('id,goods_id,goods_attr,num,price')->select();
        if($rushres){
            foreach($rushres as $v){
                if($v['goods_attr']){
                    ...
                    if(!empty($number) && $number['goods_number'] >= $v['num']){
                        ...
                        Db::name('seckill')->update(array('checked'=>2,'id'=>$v['id']));

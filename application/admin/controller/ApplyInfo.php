...
class ApplyInfo extends Common{ 
    ...
    public function complete(){
        if(request()->isPost()){
            if(input('post.id')){
                $userId = (int) input('post.user_id');
                if(empty($userId)){
                    $value = array('status'=>0,'mess'=>'请填写APP用户ID');
                    return json($value);
                }
                $user = db('member')->find($userId);
                if(empty($user)){
                    $value = array('status'=>0,'mess'=>'不存在ID为'.$userId.'的用户');
                    return json($value);
                }
                if($user['shop_id']){
                    $value = array('status'=>0,'mess'=>'ID为'.$userId.'的用户已开通过店铺');
                    return json($value);
                }
                                            ...
                                            try{
                                                ...
                                                if($shop_id){
                                                    ...
                                                    if (Db::name('shop_admin')->where('phone', $members['phone'])->find()) {
                                                        return json(array('status' => 0, 'mess' => '用户【'.$userId.'】的手机号已开通过店铺'));
                                                    }
...
//                dump($applys);die;
    ...
    public function uploadifys(){
        //  echo 1;die;
            ...
            // $uploadnum = Db::name('upload_num')->where('user_id',$userId)->find();
            // if($uploadnum){
            //     $jtime = time();
                    ...
                    try{
                        ...
                        if($uploadnum){
                            if($jtime < $uploadnum['time_out']){
                                ...
                                // Db::name('upload_num')->update(array('num'=>$cishu,'user_id'=>$userId,'id'=>$uploadnum['id']));
                            }else{
                                $time_out = strtotime(date('Y-m-d',time()))+3600*24;
                                $cishu = 1;
                                // Db::name('upload_num')->update(array('time_out'=>$time_out,'num'=>$cishu,'user_id'=>$userId,'id'=>$uploadnum['id']));
                            }
                        }else{
                            $time_out = strtotime(date('Y-m-d',time()))+3600*24;
                            // Db::name('upload_num')->insert(array('time_out'=>$time_out,'num'=>1,'user_id'=>$userId));
                        }
                        // 提交事务
                        Db::commit();

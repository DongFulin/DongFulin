    ...
    public function payment(){
        if(request()->isPost()){
            if(input('post.order_id')){
                $shop_id = session('shop_id');
                $order_id = input('post.order_id');
                $orders = Db::name('order')->where('id',$order_id)->where('shop_id',$shop_id)->where('state',1)->where('user_dakuan_status',0)->where('order_status',3)->field('id,goods_price,user_id,shop_id')->find();
                ...
                if($orders){
                    ...
                    try{
                        ...
                        $detailData['price']       =    $orders['goods_price'];
                        ...
                        Db::name('shop_wallet')->where('shop_id',$orders['shop_id'])->setInc('price', $detailData['price']);
                        //处理积分商品退还积分

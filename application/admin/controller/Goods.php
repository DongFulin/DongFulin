...
class Goods extends Common
{
    public function lst()
    {
        ...
        $list     = Db::name('goods')->alias('a')->field('a.id,a.goods_name,a.thumb_url,a.market_price,a.shop_price,a.onsale,a.is_live,b.cate_name')->join('sp_category b', 'a.cate_id = b.id', 'LEFT')->where($where)->where('a.is_recycle', 0)->order('a.addtime desc')->paginate(25);
        $page     = $list->render();
    ...
    public function uploadify()
    {
        ...
        if ($file) {
            $info = $file->validate(['size' => 3145728, 'ext' => 'jpg,png,gif,jpeg'])->move(ROOT_PATH . 'public' . DS . 'uploads' . DS . 'goods');
            if ($info) {
                ...
                $date     = date('Ymd', time());
                $original = 'uploads/goods/' . $info->getSaveName();
    ...
    public function uploadifys()
    {
        ...
        if ($file) {
            ...
            if ($info) {
                $original = 'uploads/goods_pic/' . $info->getSaveName();
                $image    = \think\Image::open('./' . $original);
                $image->thumb(640, 400)->save('./' . $original, null, 90);
...
//批量修改分类
    public function setcate()
    {
        if (request()->isAjax()) {
            ...
            $goods = db('goods');
            if ($data['cate_id'] !== '' || $data['shcate_id'] !== '') {
                if ($data['cate_id'] !== '') {
                    foreach ($ids as $k => $v) {
                        $res = $goods->where('id', $v)->update(['cate_id' => $data['cate_id']]);
                    }
                }
                if ($data['shcate_id'] !== '') {
                    foreach ($ids as $k => $v) {
                        $res = $goods->where('id', $v)->update(['shcate_id' => $data['shcate_id']]);
                    }
                }
                $arr['error'] = 0;
                return json($arr);

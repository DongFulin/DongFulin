...
class Member extends Common{
    //会员列表
    public function lst(){
        $list = Db::name('member')->alias('a')->field('a.id,a.user_name,a.headimgurl,a.phone,a.integral,a.oauth,a.regtime,a.checked,b.price')->join('sp_wallet b','a.id = b.user_id','LEFT')->order('a.regtime desc')->paginate(50)->each(function($item,$k){
            $item['headimgurl'] = url_format($item['headimgurl'],$this->webconfig['weburl']);
            $item['phone'] = substr($item['phone'],0,3).'****'.substr($item['phone'],8,3);
            return $item;
        });
    ...
    public function search(){
        ...
        $list = Db::name('member')->alias('a')->field('a.id,a.user_name,a.headimgurl,a.phone,a.integral,a.oauth,a.regtime,a.checked,b.price')->join('sp_wallet b','a.id = b.user_id','LEFT')->where($where)->order('a.regtime desc')->paginate(50);
        $count = Db::name('member')->count();

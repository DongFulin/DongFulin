        ...
        if(input('order_id')){
            ...
            if($orders){
                ...
                foreach ($order_goodres as $k => $v){
                    ...
                    $order_goodres[$k]['thumb_url'] = url_format($order_goodres[$k]['thumb_url']);
                }
    ...
    public function payment(){
        if(request()->isPost()){
            if(input('post.order_id')){
                ...
                $order_id = input('post.order_id');
                $orders = Db::name('order')->where('id',$order_id)->where('shop_id',$shop_id)->where('state',1)->where('user_dakuan_status',0)->where('order_status',3)->field('id')->find();
                if($orders){
                    // 启动事务
                    Db::startTrans();
                    try{

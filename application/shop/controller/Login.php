                            ...
                            if(input('post.repwd')){
                                ...
                                if(preg_match("/^1[3456789]{1}\\d{9}$/", $phone)){
                                    ...
                                    if($zhpwd && $zhpwd['smscode'] == $code){
                                        $valid_time = Db::name('config')->where('ca_id',2)->where('ename','mess_valid_time')->field('value')->find();
                                        $time = time();
                                        if(floor($time-$zhpwd['qtime']) <= $valid_time['value']*60){

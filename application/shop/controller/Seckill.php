...
//                if(count($optionStock) != $length){
//                    datamsg(0,'规格库存不能为空');
//                }
//                if(count($optionPrice) != $length){
//                    datamsg(0,'规格价格不能为空');
//                }
...
//                }
    ...
    public function info(){
        if(input('id')){
            ...
            $rushs = Db::name('seckill')->alias('a')->field('a.*,b.hasoption,b.goods_name,b.shop_price,b.min_price,b.max_price,b.thumb_url,c.shop_name')->join('sp_goods b','a.goods_id = b.id','LEFT')->join('sp_shops c','a.shop_id = c.id','LEFT')->where('a.id',input('id'))->where('a.shop_id',$shop_id)->find();
            if($rushs){
                if($rushs['hasoption']){
                    $goodsCommonModel = new GoodsCommonModel();

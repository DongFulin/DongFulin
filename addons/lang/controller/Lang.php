...
class Lang extends Controller{
    ...
    public function add() {
        if (request()->isPost()) {
            $data = input('post.');
	        $res=new \addons\lang\validate\Lang;
	        $result = $res->check($data);
            if(true !== $result){
	...
	public function install()
	{
		...
		$id=input('id');
		$plugin=db('plugin');
		$name=$plugin->where('id',$id)->value('name');
		$sql=$this->getPlugInfo($name,$type="/data.sql");
		dump($sql);die;
		$data=Db::execute($sql);
		$isTable = db()->query('SHOW TABLES LIKE "cmf_plugin_modules"');  //生成插件需要的数据表
		if($isTable){
			$data=db('plugin')->where('id',$id)->update(['status'=>1]);
			if($data){
				return json(array('status' => 1, 'mess' => '安装插件成功'));
			}
		}else{
			return json(array('status' => 0, 'mess' => '安装插件成功'));
		}
	}
